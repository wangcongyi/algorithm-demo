...
这是 [letcode](https://leetcode-cn.com/problems/regular-expression-matching/) 上的算法题 
...
const regularExpressionMatching = (string, pattern) => {
  ...
  for (let columnIndex = 1; columnIndex <= pattern.length; columnIndex += 1) {
    ...
    matchMatrix[0][columnIndex] = pattern[patternIndex] === ZERO_OR_MORE_CHARS ?
                                  matchMatrix[0][columnIndex - 2] : false
  }
